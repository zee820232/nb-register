...
class RegistrationFlow:
    def __init__(self, request, shutdown_event: threading.Event):
        ...
        self.thread = threading.Thread(target=self._run, name=f"browser-reg-{self.flow_id[:8]}", daemon=True)
    ...
    def _wait_for_otp(self, timeout: int = 60) -> str:
        ...
        with self._lock:
            ...
            self._otp = ""
        if not otp:
            raise TimeoutError("OTP is empty")
...
class BrowserRegistrationServicer(browser_pb2_grpc.BrowserRegistrationServicer):
    ...
    def _new_flow(self, request) -> RegistrationFlow | None:
        with self._lock:
            ...
            if self._flows:
                return None
            flow = RegistrationFlow(request, self._shutdown_event)
            self._flows[flow.flow_id] = flow
            return flow

    def _get_flow(self, flow_id: str) -> RegistrationFlow | None:
        with self._lock:
            return self._flows.get(flow_id)

    @staticmethod
    def _register_response_from_flow(flow: RegistrationFlow) -> browser_pb2.RegisterResponse:
        if not flow.done:
            return browser_pb2.RegisterResponse(
                success=False,
                error_message="browser registration did not complete",
            )
